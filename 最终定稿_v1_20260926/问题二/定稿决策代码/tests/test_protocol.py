...
class SplitTests(unittest.TestCase):
    ...
    def test_refit_epochs_use_cv_median_without_hidden_floor(self):
        self.assertEqual(fixed_epochs_from_cv([7, 8, 7], 1, 28), 7)
        self.assertEqual(fixed_epochs_from_cv([4, 5, 3], 1, 28), 4)
        self.assertEqual(fixed_epochs_from_cv([3, 2, 3], 1, 28), 3)

    def test_preflight_declares_transitive_ensemble_dependency(self):
        config = read_json(ROOT / "configs" / "protocol.json")
        scripts = config["integrity"]["ds_script_sha256"]
        self.assertIn("ds/scripts/q2_eval_robustness.py", scripts)
        self.assertFalse(config["correction"]["reuse_cv_evidence"])
...
class SelectionTests(unittest.TestCase):
    def _write_cache(
        ...
        np.savez_compressed(
            ...
            checkpoint_sha256=np.asarray(checkpoint_sha256 or member_id.zfill(64)[:64]),
            ids=np.asarray(ids),
    ...
    def test_candidate_selection_and_confirmation(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                confirmation["selection_sha256"],
                hashlib.sha256(selection_path.read_bytes()).hexdigest(),
            )
            with np.load(confirm_paths[-1], allow_pickle=False) as archive:
                payload = {key: np.asarray(archive[key]) for key in archive.files}
            payload["checkpoint_sha256"] = np.asarray("different-checkpoint")
            np.savez_compressed(confirm_paths[-1], **payload)
            with self.assertRaisesRegex(RuntimeError, "checkpoint mismatch"):
                confirm_ensemble(
                    ...
                    data,
                    selection_path,
                    root / "confirmation_mismatch.json",
                )
