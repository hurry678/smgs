...
def choose_subset(sid: str, epoch: int, seed: int) -> Tuple[int, ...]:
    u = stable_unit(f"{sid}|subset|{epoch}|{seed}")
    if u < 0.70:
        return (int(stable_unit(f"{sid}|single|{epoch}|{seed}") * 3),)
    if u < 0.95:
        choices = [(0, 1), (0, 2), (1, 2)]
        return choices[int(stable_unit(f"{sid}|double|{epoch}|{seed}") * 3)]
    return (0, 1, 2)
...
def train_one(kind: str, seed: int, train: Dict[str, Any], valid: Dict[str, Any], args: argparse.Namespace, device: torch.device) -> Tuple[Q2Model, Dict[str, Any]]:
    set_seed(seed)
    model = Q2Model(kind, args.model_dir, hidden=args.hidden, dropout=args.dropout).to(device)
