...
def run_local_occlusion(
    ...
    for pos, gidx in enumerate(sample_indices.tolist()):
        for m, modality in enumerate(MODALITIES):
            ...
            for wi, (start, end, actual_positions) in enumerate(specs):
                ...
                # Avoid a degenerate scattered baseline that is itself one
                # contiguous block; this keeps the comparison meaningful.
                for _ in range(8):
    ...
    for pos, gidx in enumerate(sample_indices.tolist()):
        for m, modality in enumerate(MODALITIES):
            actual = [r for r in records if r["sample_position"] == pos and r["modality_index"] == m and r["pattern"] == "continuous"]
            if not actual:
                continue
            actual.sort(key=lambda r: r["abs_class_delta"], reverse=True)
            rand = [r for r in records if r["sample_position"] == pos and r["modality_index"] == m and r["pattern"] == "random_contiguous"]
            scat = [r for r in records if r["sample_position"] == pos and r["modality_index"] == m and r["pattern"] == "point_scatter"]
            wins_r = sum(a["abs_class_delta"] > b["abs_class_delta"] for a, b in zip(actual, rand))
            wins_s = sum(a["abs_class_delta"] > b["abs_class_delta"] for a, b in zip(actual, scat))
            top_actual = actual[0]
            sum_abs = float(sum(r["abs_class_delta"] for r in actual))
            summaries.append({
                "sample_position": pos,
                "global_index": int(gidx),
                "modality": modality,
                ...
                "top1_concentration": float(top_actual["abs_class_delta"] / sum_abs) if sum_abs > 0 else 0.0,
                "mean_abs_class_delta": float(np.mean([r["abs_class_delta"] for r in actual])),
                "mean_abs_raw_delta": float(np.mean([r["abs_raw_delta"] for r in actual])),
                "mean_random_abs_class_delta": float(np.mean([r["abs_class_delta"] for r in rand])) if rand else 0.0,
                "mean_scatter_abs_class_delta": float(np.mean([r["abs_class_delta"] for r in scat])) if scat else 0.0,
                "wins_vs_random_contiguous": int(wins_r),
                "wins_vs_point_scatter": int(wins_s),
            })
    agg: Dict[str, Any] = {}
    ...
    total_windows = sum(x["n_windows"] for x in summaries)
    agg["continuous_win_rate_vs_random_contiguous"] = (
        float(sum(x["wins_vs_random_contiguous"] for x in summaries) / total_windows) if total_windows else 0.0
    )
    agg["continuous_win_rate_vs_point_scatter"] = (
        float(sum(x["wins_vs_point_scatter"] for x in summaries) / total_windows) if total_windows else 0.0
    )
...
def build_sample_explanations(
    ...
    for pos, gidx in enumerate(sample_indices.tolist()):
        ...
        primary = MODALITIES[int(np.argmax(combined))] if combined.max() > 0 else None
        evidence: Dict[str, List[Dict[str, Any]]] = {}
        for m, modality in enumerate(MODALITIES):
            rows = [x for x in local_summaries if x["sample_position"] == pos and x["modality"] == modality]
            rows.sort(key=lambda x: x["top_abs_class_delta"], reverse=True)
            evidence[modality] = []
            for row in rows[:top_k]:
                mapped = map_evidence(modality, row["top_positions"], int(gidx), data, offsets, audit_map)
                evidence[modality].append({
                    "window_index": int(row["top_window_index"]),
                    "positions": list(row["top_positions"]),
                    "class_delta": float(row["top_class_delta"]),
                    "abs_class_delta": float(row["top_abs_class_delta"]),
                    "raw_delta": float(row["top_raw_delta"]),
                    "abs_raw_delta": float(row["top_abs_raw_delta"]),
                    **mapped,
                })
        has_evidence = bool(any(evidence[x] for x in MODALITIES))
        no_evidence_reason = None if has_evidence else "no valid local evidence window"
        out.append({
...
def run_validation(args: argparse.Namespace, device: torch.device) -> None:
    ...
    _, local_summaries, local_agg = run_local_occlusion(
    ...
    explanations = build_sample_explanations(
        ...
        class_values, raw_values, local_summaries, offsets, {}, args.top_k,
    )
    report = {
        "mode": "validation",
        "protocol": {
...
def run_attachment4(args: argparse.Namespace, device: torch.device) -> None:
    ...
    explanations = build_sample_explanations(
        ...
        class_values, raw_values, local_summaries, offsets, audit_map, args.top_k,
    )
    report = {
        "mode": "attachment4",
        "protocol": {
