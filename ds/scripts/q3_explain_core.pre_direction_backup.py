...
def build_sample_explanations(
    ...
    for pos, gidx in enumerate(sample_indices.tolist()):
        ...
        for m, modality in enumerate(MODALITIES):
            ...
            rows = [x for x in rows if max(float(x["top_abs_class_delta"]), float(x["top_abs_raw_delta"])) > 1e-8]
            if rows:
                class_ranks = percentile_rank([float(x["top_abs_class_delta"]) for x in rows])
                raw_ranks = percentile_rank([float(x["top_abs_raw_delta"]) for x in rows])
                for row, cr, rr in zip(rows, class_ranks, raw_ranks):
                    row["_rank_score"] = 0.5 * (float(cr) + float(rr))
                rows.sort(
                    key=lambda x: (float(x["_rank_score"]), float(x["top_abs_class_delta"]), float(x["top_abs_raw_delta"])),
                    reverse=True,
                )
            evidence[modality] = []
            for row in rows[: int(top_k)]:
                mapped = map_evidence(modality, row["top_positions"], int(gidx), data, offsets, audit_map)
                score = float(row.get("_rank_score", 0.0))
                beat = float(row.get("control_beat_rate", 0.0))
                ...
                valid_any = True
        no_evidence_reason = None if valid_any else "no local occlusion window exceeded the numerical evidence threshold"
        out.append({
...
def run_validation(args: argparse.Namespace, device: torch.device) -> None:
    ...
    report = {
        ...
        "protocol": {
            ...
            "evidence_ranking_rule": "per sample-modality average percentile rank of continuous-window absolute class and raw-intensity occlusion effects; top-k retained",
