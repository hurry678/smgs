...
def main() -> None:
    ...
    for p in pkls:
        ...
        if not np.isfinite(tf).all() or not np.isfinite(au).all() or not np.isfinite(vi).all():
            errors.append(f'{sid}: non-finite feature value')

        enc = tok(
            raw, add_special_tokens=True, truncation=True, max_length=50,
        ...
        for i, off in enumerate(token_offsets):
            ...
            position_to_time.append({
                ...
                'char_end': int(b),
                'time_center': t,
                'time_start_est': max(0.0, t - 0.5 * (float(duration) / max(1, len(raw))) * max(1, b - a)),
                'time_end_est': min(float(duration), t + 0.5 * (float(duration) / max(1, len(raw))) * max(1, b - a)),
        ...
        metadata_samples.append({
            ...
            'video_file': str(video_path.relative_to(args.raw_dir)) if video_path.exists() else None,
            ...
            'audio_valid_positions': [int(i) for i in np.flatnonzero(np.any(np.abs(au) > 1e-8, axis=1))],
            'vision_valid_positions': [int(i) for i in np.flatnonzero(np.any(np.abs(vi) > 1e-8, axis=1))],
    ...
    (args.out_dir / 'attachment4_audit.json').write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding='utf-8')
    print(json.dumps({
        'status': audit['status'],
        'n': len(ids),
        'errors': errors,
