...
- [x] G4（问题二部分）：固定缺失协议，训练强基线和M0，完成主矩阵、位置分析、M1/M2失败对照和等预算比较。
- [ ] G5：实现对应实际发布目标的归因、Q3基础评价；训练素材验证映射方法，验证集检验数值解释。
- [x] G6（问题二部分）：完成核心消融与多种子比较；M1/M2被拒绝，最终保留M0结构并用9检查点集成降低随机性。
- [ ] G7：冻结所有模型、规则、阈值、版本与强基线/test比较清单。
  - [x] 问题二部分已完成：`M3-ensemble9 + C2`于2026-09-24 18:12:54 CST冻结，版本清单、脚本、检查点和哈希均已登记。
- [ ] G8：一次正式test评估，完成两类专项全量结果与解释，并核对20条原素材定位。
  - [x] 问题二部分已完成：官方test冻结后单次评价，附件3共30条冻结推理，CSV/JSON一致性及SHA-256已核验。
...
## 当前版本状态（2026-09-24 18:19:39 CST）

问题二当前正式版本已从`M0_seed42 + C2`更新为`M3-ensemble9 + C2`：

```text
成员：M0 seeds 42–50，共9个检查点
融合：softmax概率平均 + raw intensity平均
验证集R_MAE：0.604523（旧版0.610839）
冻结时间：2026-09-24 18:12:54 CST
发布时间：2026-09-24 18:15:39 CST
正式附件3：submission/q2_predictions.csv，30条
```

官方test为冻结后单次评价。M3相对旧版呈现混合结果：强度MAE从`0.653008`降至`0.643349`、Pearson从`0.643591`升至`0.657517`，但Accuracy从`0.639615`降至`0.635488`、Macro-F1从`0.609117`降至`0.596337`。test结果未用于反向选型或调参，论文中不得宣称M3在test上全面优于旧版。

旧版`M0_seed42 + C2`完整保留在：
...
artifacts/q2/v3_m3_ensemble9/publication_manifest.json
artifacts/q2/v3_m3_ensemble9/test/test_test_evaluation.json
artifacts/q2/v3_m3_ensemble9/submission/attachment3_predictions.csv
docs/问题二_全过程记录.md
```
