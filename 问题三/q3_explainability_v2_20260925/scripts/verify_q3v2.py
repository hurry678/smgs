#!/usr/bin/env python3
"""Verify Q3 v2 source integrity, frozen inference, and final submission."""
...
def main() -> None:
    ...
    protocol_config = json.loads(
        (args.extension_root / "configs" / "protocol.json").read_text(encoding="utf-8")
    )
    ...
    if manifest.get("status") != "completed_with_frozen_attachment4_inference":
    ...
    if baseline_diff.returncode != 0:
        raise SystemExit("protected DS/Q2 baseline paths differ from HEAD")
