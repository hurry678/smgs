...
REPO_ROOT = Path(
    os.environ.get("Q3V2_REPO_ROOT", Path(__file__).resolve().parents[3])
).resolve()
DS_SCRIPTS = REPO_ROOT / "ds" / "scripts"
if str(DS_SCRIPTS) not in sys.path:
    sys.path.insert(0, str(DS_SCRIPTS))

from q2_eval import POLARITY, choose_device  # noqa: E402
from q2_ensemble_publish import load_models  # noqa: E402
from q2_train import apply_normalization, load_split, metrics, publish_c2  # noqa: E402
...
SUBSETS = coalition_subsets()
FULL_MASK = 7
validate_coalition_order(SUBSETS)
...
def load_q2_data(
    path: Path, split: str, norm: Dict[str, torch.Tensor], device: torch.device
) -> Dict[str, Any]:
...
def load_attachment4(
    path: Path, norm: Dict[str, torch.Tensor], device: torch.device
) -> Dict[str, Any]:
    with np.load(path, allow_pickle=False) as z:
        text_bert = torch.from_numpy(np.asarray(z["text_bert"], dtype=np.int64)).to(
            device
        )
...
def ensemble_batch_scores(
    ...
    cls_score = torch.log(
        mean_prob.gather(1, target_classes[:, None]).clamp_min(1e-9)
    ).squeeze(1)
    return (
        cls_score.detach().cpu().numpy(),
        mean_raw.detach().cpu().numpy(),
        mean_prob.detach().cpu().numpy(),
    )


def run_ensemble_outputs(
    models: Sequence[Any], data: Dict[str, Any], batch_size: int = 64
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    n = len(data["ids"])
    ...
    for start in range(0, n, batch_size):
        ...
        obs = data["obs"][idx]
        _, raw, prob = ensemble_batch_scores(models, data, obs, idx, None)
...
def shapley3(values: np.ndarray) -> np.ndarray:
    """Exact Shapley for three players in integer bit-mask coalition order."""
    return exact_shapley(values)


def loo_values(values: np.ndarray) -> np.ndarray:
    return exact_loo(values)
...
def modality_values(
    ...
    for start in range(0, len(idx_cat), batch_size):
        ...
        cls, raw, _ = ensemble_batch_scores(
            models,
            data,
            obs_cat[start:end].to(device),
            idx_cat[start:end],
            target_cat[start:end],
        )
...
def window_specs(
    positions: np.ndarray, window_size: int, stride: int
...
def evaluate_perturbations(
    ...
    for start in range(0, len(records), batch_size):
        ...
        idx = torch.as_tensor(
            [r["global_index"] for r in chunk], dtype=torch.long, device=device
        )
        target = torch.as_tensor(
            [r["target_class"] for r in chunk], dtype=torch.long, device=device
        )
        obs = torch.stack([r["obs"] for r in chunk], dim=0).to(device)
        cls, raw, _ = ensemble_batch_scores(models, data, obs, idx, target)
        cls_out.append(cls)
        raw_out.append(raw)
    return np.concatenate(cls_out).astype(np.float32), np.concatenate(raw_out).astype(
        np.float32
    )


def paired_control_stats(
    continuous: Sequence[float], control: Sequence[float], seed: int, n_boot: int = 5000
) -> Dict[str, Any]:
    ...
    if sign_n == 0:
        ...
        log_terms = [
            math.lgamma(sign_n + 1.0)
            - math.lgamma(i + 1.0)
            - math.lgamma(sign_n - i + 1.0)
            - log_den
            for i in range(k + 1)
        ]
...
def top_support_fidelity(
    records: Sequence[Dict[str, Any]],
    sample_ids: Sequence[str],
    seed: int,
) -> Dict[str, Any]:
    """Compare the strongest signed deletion against matched-budget controls."""
    by_key = {
        (
            int(row["sample_position"]),
            int(row["modality_index"]),
            str(row["pattern"]),
    ...
    keys = sorted(
        {
            (int(row["sample_position"]), int(row["modality_index"]))
            for row in records
            if row["pattern"] == "continuous"
        }
    )
    for sample_position, modality_index in keys:
        candidates = [
            row
            for row in records
            if int(row["sample_position"]) == sample_position
            and int(row["modality_index"]) == modality_index
            and row["pattern"] == "continuous"
        ]
        top = max(
            candidates,
            key=lambda row: (float(row["class_delta"]), -int(row["window_index"])),
        )
        budget = max(1, len(top["positions"]))
        normalized.append(float(top["class_delta"]) / budget)
        groups.append(_sample_group(sample_ids[int(top["global_index"])]))
        for pattern in comparisons:
            control = by_key[
                (
                    ...
                    modality_index,
                    pattern,
                    int(top["window_index"]),
                )
            ]
            comparisons[pattern].append(
                float(top["class_delta"]) - float(control["class_delta"])
            )
    output: Dict[str, Any] = {
        "n_sample_modalities": len(normalized),
        "mean_budget_normalized_top_support": (
            float(np.mean(normalized)) if normalized else 0.0
        ),
    }
    for offset, (pattern, differences) in enumerate(comparisons.items()):
...
def run_local_occlusion(
        ...
        for m, modality in enumerate(MODALITIES):
            ...
            for wi, (start, end, actual_positions) in enumerate(specs):
                ...
                for _ in range(8):
                    if (
                        k <= 1
                        or len(np.unique(np.diff(scatter_positions))) > 1
                        or len(positions) <= k
                    ):
                        break
                    scatter_idx = rng.choice(len(positions), size=k, replace=False)
    ...
    by_key = {
        (
            int(r["sample_position"]),
            int(r["modality_index"]),
            str(r["pattern"]),
            int(r["window_index"]),
        ): r
        for r in records
    ...
    for pos, gidx in enumerate(sample_indices.tolist()):
        for m, modality in enumerate(MODALITIES):
            actual_all = [
                r
                for r in records
                if r["sample_position"] == pos
                and r["modality_index"] == m
                and r["pattern"] == "continuous"
            ]
            if not actual_all:
                continue
            ...
            control_vals = np.asarray(
                [
                    ...
                    if r["sample_position"] == pos
                    and r["modality_index"] == m
                    and r["pattern"] in ("random_contiguous", "point_scatter")
                ],
                dtype=np.float64,
            )
            control_beat_rate = (
                float(np.mean(control_vals <= top_actual["abs_class_delta"]))
                if len(control_vals)
                else 0.0
            )
            random_rows = [
            ...
            summaries.append(
                {
                    ...
                    "mean_abs_class_delta": float(
                        np.mean([r["abs_class_delta"] for r in actual_all])
                    ),
                    "mean_abs_raw_delta": float(
                        np.mean([r["abs_raw_delta"] for r in actual_all])
                    ),
                    "control_beat_rate": control_beat_rate,
                    "random_top1_concentration": top1_concentration(random_rows)
                    if random_rows
                    else 0.0,
                    "scatter_top1_concentration": top1_concentration(scatter_rows)
                    if scatter_rows
                    else 0.0,
                    "wins_vs_random_contiguous": int(
                        sum(
                            float(a["abs_class_delta"])
                            > float(b["abs_class_delta"]) + 1e-8
                            for a, b in zip(actual_all, random_rows)
                        )
                    ),
                    "wins_vs_point_scatter": int(
                        sum(
                            float(a["abs_class_delta"])
                            > float(b["abs_class_delta"]) + 1e-8
    ...
    cont_pairs = [
        by_key[(p, m, "continuous", w)]
        for p, m, w in paired_keys
        if (p, m, "continuous", w) in by_key
    ]
    rand_pairs = [
        by_key[(p, m, "random_contiguous", w)]
        for p, m, w in paired_keys
        if (p, m, "random_contiguous", w) in by_key
    ]
    scat_pairs = [
        by_key[(p, m, "point_scatter", w)]
        for p, m, w in paired_keys
        if (p, m, "point_scatter", w) in by_key
    ]
    ...
    stats_r = paired_control_stats(
        [a["abs_class_delta"] for a, _ in valid_r],
        [b["abs_class_delta"] for _, b in valid_r],
        seed=seed + 17011,
    )
    stats_s = paired_control_stats(
        [a["abs_class_delta"] for a, _ in valid_s],
        [b["abs_class_delta"] for _, b in valid_s],
        seed=seed + 27011,
    )
    ...
    agg["mean_top1_concentration_continuous"] = (
        float(np.mean([x["top1_concentration"] for x in summaries]))
        if summaries
        else 0.0
    )
    agg["mean_top1_concentration_random_contiguous"] = (
        float(np.mean([x["random_top1_concentration"] for x in summaries]))
        if summaries
        else 0.0
    )
    agg["mean_top1_concentration_point_scatter"] = (
        float(np.mean([x["scatter_top1_concentration"] for x in summaries]))
        if summaries
        else 0.0
    )
    agg["top_support_fidelity"] = top_support_fidelity(
        records,
        data["ids"],
        seed,
    )
            ...
            for row, class_rank, raw_rank in zip(actual_all, class_ranks, raw_ranks):
                ...
                beat = (
                    float(
                        np.mean(
                            [
                                float(c["abs_class_delta"])
                                <= float(row["abs_class_delta"]) + 1e-8
                                for c in controls
                            ]
                        )
                    )
...
def run_point_scan(
    ...
    """Single-position occlusion scan on a deterministic spread of the subset."""
    records: List[Dict[str, Any]] = []
    maskable = data["maskable_obs"].detach().cpu().numpy()
    take = min(int(point_n), len(sample_indices))
    chosen_positions = (
        np.linspace(0, len(sample_indices) - 1, num=take, dtype=np.int64)
        if take
        else np.zeros((0,), dtype=np.int64)
    )
    for pos in chosen_positions.tolist():
    ...
    for pos in chosen_positions.tolist():
        for m, modality in enumerate(MODALITIES):
            rows = [
                r
                for r in records
                if r["sample_position"] == pos and r["modality_index"] == m
            ]
            if not rows:
                continue
            ...
            summaries.append(
                {
                    ...
                    "mean_abs_class_delta": float(
                        np.mean([r["abs_class_delta"] for r in rows])
                    ),
                }
            )
    agg = {
        "n_samples": take,
        "sample_positions": [int(x) for x in chosen_positions.tolist()],
        "n_perturbations": len(records),
        "mean_top_abs_class_delta": float(
            np.mean([x["top_abs_class_delta"] for x in summaries])
        )
        if summaries
        else 0.0,
        "mean_abs_class_delta": float(np.mean([r["abs_class_delta"] for r in records]))
        if records
        else 0.0,
        "mean_top_abs_raw_delta": float(
            np.mean([x["top_abs_raw_delta"] for x in summaries])
        )
        if summaries
        else 0.0,
    }
...
def map_evidence(
    ...
    if modality == "text":
        span = evidence_character_span(
            offsets["offsets"][global_index],
            offsets["valid"][global_index],
            positions,
        )
        if span is not None:
            start, end = span
    ...
    if rows:
        start_key = (
            "time_start_seconds"
            if all("time_start_seconds" in row for row in rows)
            else "time_start_est"
        )
        end_key = (
            "time_end_seconds"
            if all("time_end_seconds" in row for row in rows)
            else "time_end_est"
        )
        statuses = {str(row.get("status", "unknown")) for row in rows}
        if statuses <= {"mfa_forced_alignment"}:
            mapping_status = "forced_alignment"
            precision = "mfa_word_timing_mapped_to_bert_tokens"
            warning = (
                "Automatic forced alignment; timestamps are model-derived and "
                "not manual ground truth."
            )
        else:
            mapping_status = "approximate"
            precision = "proportional_fallback"
            warning = (
                "No usable forced alignment was available; timestamps are "
    ...
    if modality == "audio":
        if rows:
            return {
                ...
                "time_start_seconds": float(min(float(x[start_key]) for x in rows)),
                "time_end_seconds": float(max(float(x[end_key]) for x in rows)),
                "mapping_confidence": float(
                    np.mean([float(x.get("confidence", 0.0)) for x in rows])
                ),
                "mapping_confidence_definition": str(
                    rows[0].get("confidence_definition", "unspecified")
                ),
                "token_positions": positions,
                "warning": warning,
            }
    ...
    if rows:
        frame_starts = [
            int(x["frame_start"]) for x in rows if x.get("frame_start") is not None
        ]
        frame_ends = [
            int(x["frame_end"]) for x in rows if x.get("frame_end") is not None
        ]
        if not frame_starts:
            frame_starts = [
                int(x["frame_center_est"])
                for x in rows
                if x.get("frame_center_est") is not None
            ]
            frame_ends = list(frame_starts)
        return {
            "mapping_status": mapping_status,
            "mapping_precision": precision + "_and_video_frames",
            "time_start_seconds": float(min(float(x[start_key]) for x in rows)),
            "time_end_seconds": float(max(float(x[end_key]) for x in rows)),
            "frame_start_est": int(min(frame_starts)) if frame_starts else None,
            "frame_end_est": int(max(frame_ends)) if frame_ends else None,
            "mapping_confidence": float(
                np.mean([float(x.get("confidence", 0.0)) for x in rows])
            ),
            "mapping_confidence_definition": str(
...
def build_sample_explanations(
    ...
    class_interactions = exact_pairwise_interactions(class_values)
    raw_interactions = exact_pairwise_interactions(raw_values)
    out: List[Dict[str, Any]] = []
    for pos, gidx in enumerate(sample_indices.tolist()):
        c_abs = np.abs(class_phi[pos])
        r_abs = np.abs(raw_phi[pos])
        c_share = (
            c_abs / c_abs.sum() if c_abs.sum() > 0 else np.zeros(3, dtype=np.float32)
        )
        r_share = (
            r_abs / r_abs.sum() if r_abs.sum() > 0 else np.zeros(3, dtype=np.float32)
        )
        modality_effect = 0.5 * (c_share + r_share)
        primary_idx, strongest_idx, primary_basis = primary_reference(class_phi[pos])
        support = (
            MODALITIES[primary_idx]
            if float(class_phi[pos, primary_idx]) > 0.0
            else None
        )
        ...
        for modality in MODALITIES:
            rows = [
                x
                for x in window_summaries
                if int(x["sample_position"]) == pos and x["modality"] == modality
            ]
            for row in rows:
                row["directional_intensity_delta"] = directional_intensity_delta(
                    int(pred_cls[gidx]),
                    float(raw[gidx]),
                    float(row["raw_delta"]),
                )
            class_ranks = percentile_rank([float(x["class_delta"]) for x in rows])
            intensity_ranks = percentile_rank(
                [float(x["directional_intensity_delta"]) for x in rows]
            )
            for row, class_rank, intensity_rank in zip(
                rows, class_ranks, intensity_ranks
            ):
                row["evidence_rank_score"] = float(
                    0.7 * class_rank + 0.3 * intensity_rank
                )
            ...
            for row in support_rows[: int(top_k)]:
                mapped = map_evidence(
                    modality, row["positions"], int(gidx), data, offsets, audit_map
                )
                ...
                evidence[modality].append(
                    {
                        ...
                        "raw_delta": float(row["raw_delta"]),
                        "abs_raw_delta": float(row["abs_raw_delta"]),
                        "directional_intensity_delta": float(
                            row["directional_intensity_delta"]
                        ),
        ...
        no_evidence_reason = (
            None
            if valid_support_any
            else "no positive continuous-window contribution exceeded the numerical evidence threshold"
        )
        out.append(
            {
                ...
                "primary_modality_basis": primary_basis,
                ...
                "class_pair_interactions": {
                    name: float(class_interactions[pos, index])
                    for index, name in enumerate(PAIR_NAMES)
                },
                "intensity_pair_interactions": {
                    name: float(raw_interactions[pos, index])
                    for index, name in enumerate(PAIR_NAMES)
                },
                "class_shapley_sum_error": float(
                    abs(
                        class_phi[pos].sum()
                        - (class_values[pos, FULL_MASK] - class_values[pos, 0])
                    )
                ),
                "intensity_shapley_sum_error": float(
                    abs(
                        raw_phi[pos].sum()
                        - (raw_values[pos, FULL_MASK] - raw_values[pos, 0])
                    )
                ),
...
def summarize_shapley(
    ...
    c_sum_err = np.abs(
        class_phi.sum(1) - (class_values[:, FULL_MASK] - class_values[:, 0])
    )
    ...
    c_interactions = exact_pairwise_interactions(class_values)
    r_interactions = exact_pairwise_interactions(raw_values)
    interaction_summary: Dict[str, Any] = {}
    for index, pair_name in enumerate(PAIR_NAMES):
        interaction_summary[pair_name] = {
            "class_mean_signed": float(np.mean(c_interactions[:, index])),
            "class_mean_absolute": float(np.mean(np.abs(c_interactions[:, index]))),
            "intensity_mean_signed": float(np.mean(r_interactions[:, index])),
            "intensity_mean_absolute": float(np.mean(np.abs(r_interactions[:, index]))),
        }
    return {
        "class_shapley_sum_error_max": float(c_sum_err.max())
        if len(c_sum_err)
        else 0.0,
        "class_shapley_sum_error_mean": float(c_sum_err.mean())
        if len(c_sum_err)
        else 0.0,
        "intensity_shapley_sum_error_max": float(r_sum_err.max())
        if len(r_sum_err)
        else 0.0,
        "intensity_shapley_sum_error_mean": float(r_sum_err.mean())
        if len(r_sum_err)
        else 0.0,
        "main_modality_agreement_shapley_vs_loo": float(np.mean(c_main == l_main))
        if len(c_main)
        else 0.0,
        "mean_abs_signed_difference_shapley_vs_loo": float(
            np.mean(np.abs(class_phi - class_loo))
        ),
        "mean_class_abs_share_text": float(
            np.mean(c_abs[:, 0] / np.clip(c_abs.sum(1), 1e-12, None))
        ),
        "mean_class_abs_share_audio": float(
            np.mean(c_abs[:, 1] / np.clip(c_abs.sum(1), 1e-12, None))
        ),
        "mean_class_abs_share_vision": float(
            np.mean(c_abs[:, 2] / np.clip(c_abs.sum(1), 1e-12, None))
        ),
        "mean_intensity_abs_share_text": float(
            np.mean(r_abs[:, 0] / np.clip(r_abs.sum(1), 1e-12, None))
        ),
        "mean_intensity_abs_share_audio": float(
            np.mean(r_abs[:, 1] / np.clip(r_abs.sum(1), 1e-12, None))
        ),
        "mean_intensity_abs_share_vision": float(
            np.mean(r_abs[:, 2] / np.clip(r_abs.sum(1), 1e-12, None))
        ),
        "pairwise_shapley_interactions": interaction_summary,
        "n": int(len(class_phi)),
    }


def verify_freeze_manifest(
    manifest_path: Path | None, checkpoints: Sequence[Path]
) -> Dict[str, Any]:
    ...
    if not ok:
        raise ValueError(
            "checkpoint SHA-256 values or order do not match the frozen M3 manifest"
        )
    return {
        "checked": True,
        "ok": bool(ok),
...
def run_validation(args: argparse.Namespace, device: torch.device) -> None:
    started = time.time()
    models, _, norm, kind, seeds = load_models(args.checkpoints, args.model_dir, device)
    ...
    if len(offsets["match"]) != len(data["ids"]) or not bool(offsets["match"].all()):
        raise ValueError("tokenizer offsets do not match the selected Q2 split")
    labels = data["labels"].detach().cpu().numpy()
    regression = data["regression"].detach().cpu().numpy()
    logits, raw, _ = run_ensemble_outputs(models, data, args.batch_size)
    ...
    class_values, raw_values = modality_values(
        models, data, selected, target, args.batch_size
    )
    class_phi = shapley3(class_values)
    raw_phi = shapley3(raw_values)
    class_loo = loo_values(class_values)
    shapley_report = summarize_shapley(
        class_phi, raw_phi, class_loo, class_values, raw_values
    )
    print(
        json.dumps({"stage": "shapley", **shapley_report}, ensure_ascii=False),
        flush=True,
    )
    _, _, local_window_summaries, local_agg = run_local_occlusion(
        models,
        data,
        selected,
        target,
    ...
    print(
        json.dumps({"stage": "local_occlusion", **local_agg}, ensure_ascii=False),
        flush=True,
    )
    _, _, point_agg = run_point_scan(
        models,
        data,
        selected,
        target,
    ...
    print(
        json.dumps({"stage": "point_scan", **point_agg}, ensure_ascii=False), flush=True
    )
    ...
    report = {
        ...
        "protocol": {
            ...
            "paired_statistics": "window-level paired diagnostics plus 5000-resample video-group clustered bootstrap for top-support fidelity",
            "primary_modality_rule": "largest positive predicted-class Shapley contribution; if none is positive, largest absolute predicted-class Shapley contribution with support_modality=null",
            "strongest_modality_rule": "largest absolute predicted-class Shapley contribution",
            "evidence_ranking_rule": "support: positive continuous-window class contribution ranked by 0.7 signed-class percentile + 0.3 class-oriented intensity percentile; counter: most negative class contribution; top-k support retained",
        ...
        "model": {
            "kind": kind,
            "seeds": seeds,
            "checkpoint_files": [x.name for x in args.checkpoints],
    ...
    (args.out_dir / "q3_validation_report.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (args.out_dir / "q3_validation_explanations.json").write_text(
    ...
    np.savez_compressed(
        ...
        raw_phi=raw_phi,
        class_loo=class_loo,
        class_pair_interactions=exact_pairwise_interactions(class_values),
        raw_pair_interactions=exact_pairwise_interactions(raw_values),
        full_logits=logits,
        full_raw=raw,
    )
    print(
...
def run_attachment4(args: argparse.Namespace, device: torch.device) -> None:
    ...
    if len(offsets["match"]) != len(data["ids"]) or not bool(offsets["match"].all()):
        raise ValueError("attachment-4 tokenizer offsets do not match stored tensors")
    if args.audit is None:
        raise ValueError("--audit is required in attachment4 mode")
    ...
    if (
        ...
        or float(alignment_audit.get("mfa_sample_rate", -1.0))
        < float(alignment_audit.get("minimum_mfa_sample_rate", 1.0))
    ):
        raise ValueError("attachment-4 MFA alignment audit did not pass")
    ...
    class_values, raw_values = modality_values(
        models, data, all_indices, target, args.batch_size
    )
    class_phi = shapley3(class_values)
    raw_phi = shapley3(raw_values)
    class_loo = loo_values(class_values)
    ...
    report = {
        ...
        "protocol": {
            ...
            "primary_modality_rule": "largest positive predicted-class Shapley contribution; if none is positive, largest absolute predicted-class Shapley contribution with support_modality=null",
            "strongest_modality_rule": "largest absolute predicted-class Shapley contribution",
            "evidence_ranking_rule": "0.7 signed-class percentile + 0.3 class-oriented intensity percentile",
        },
        "model": {
            "kind": kind,
            "seeds": seeds,
        ...
        "shapley_vs_loo": summarize_shapley(
            class_phi, raw_phi, class_loo, class_values, raw_values
        ),
        "local_occlusion": local_agg,
        "point_scan": point_agg,
        ...
        "notes": [
            ...
            "Audio and vision evidence use MFA word timings when available and explicitly marked proportional fallback otherwise.",
            "Text evidence locations are exact tokenizer character offsets.",
        ],
    }
    args.out_dir.mkdir(parents=True, exist_ok=True)
    (args.out_dir / "q3_attachment4_raw.json").write_text(
        json.dumps(
            {"report": report, "samples": explanations}, ensure_ascii=False, indent=2
        ),
        encoding="utf-8",
    )
    np.savez_compressed(
...
def main() -> None:
    ...
    ap.add_argument("--n", type=int, default=728)
    ap.add_argument("--window-size", type=int, default=5)
    ap.add_argument("--stride", type=int, default=5)
    ap.add_argument("--point-n", type=int, default=128)
