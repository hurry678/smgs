#!/usr/bin/env python3
"""Validation-only ensemble selection and confirmation."""
...
def fixed_epochs_from_cv(best_epochs: Sequence[int], minimum: int, maximum: int) -> int:
    if not best_epochs:
        raise ValueError("best_epochs cannot be empty")
    median = int(round(float(np.median(best_epochs))))
    return int(np.clip(median, minimum, maximum))
...
def select_ensemble(
    ...
    result = {
        ...
        "validation_status": config.get("correction", {}).get(
            "validation_status", "unexposed"
        ),
        "partition": {
...
def confirm_ensemble(
    ...
    if selection.get("protocol_sha256") != config.get("_runtime_protocol_sha256"):
        raise RuntimeError("selection belongs to a different protocol")
    with np.load(data_path, allow_pickle=False) as archive:
        labels = np.asarray(archive["valid_labels"], dtype=np.int64)
        regression = np.asarray(archive["valid_regression"], dtype=np.float32)
        data_ids = [str(value) for value in archive["valid_ids"].tolist()]
    ...
    expected_checkpoint_sha = {
        item["id"]: item["sha256"] for item in config["baseline"]["checkpoints"]
    }
    expected_checkpoint_sha.update(
        {
            member_id: record["checkpoint_sha256"]
            for member_id, record in selection["selected"]["members"].items()
        }
    )
    mismatched = sorted(
        member_id
        for member_id in required
        if members[member_id]["checkpoint_sha256"] != expected_checkpoint_sha[member_id]
    )
    if mismatched:
        raise RuntimeError(f"confirmation checkpoint mismatch: {mismatched}")
    baseline_arrays = ensemble_arrays(members, baseline_weights)
